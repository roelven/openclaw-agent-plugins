#!/usr/bin/env node
/**
 * openclaw scores CLI — analyze session quality scores.
 *
 * Usage:
 *   npx tsx scores-cli.ts summary          # 7-day averages
 *   npx tsx scores-cli.ts trend --days 30  # Score trend over time
 *   npx tsx scores-cli.ts worst --n 5      # Worst N sessions
 *   npx tsx scores-cli.ts compare          # Compare models
 *
...
function loadScores(): SessionScore[] {
  try {
    const files = readdirSync(SCORES_DIR).filter((f) => f.endsWith(".json") && f !== "scores-cli.ts");
    return files
      .map((f) => {
        try {
          return JSON.parse(readFileSync(join(SCORES_DIR, f), "utf-8")) as SessionScore;
        } catch {
          return null;
        }
      })
...
function filterByDays(scores: SessionScore[], days: number): SessionScore[] {
...
function summary(scores: SessionScore[], json: boolean) {
...
function trend(scores: SessionScore[], days: number, json: boolean) {
...
function worst(scores: SessionScore[], n: number, json: boolean) {
...
switch (command) {
  ...
  case "worst":
    ...
    break;
  default:
    console.log("Usage: scores-cli.ts <summary|trend|worst> [--json] [--days N] [--n N]");
    process.exit(1);
}
